...
STEER_DEADZONE = 0.10 # Ignoro i piccoli movimenti dello stick al centro
...
class XboxController:
    ...
    def __init__(self):
        # Permette a Pygame di leggere il controller anche se la finestra del terminale non è in primo piano
        os.environ["SDL_JOYSTICK_ALLOW_BACKGROUND_EVENTS"] = "1"
        pygame.init()
        pygame.joystick.init()

        # Cerca quanti joystick sono collegati al PC
        ...
        # Serve per capire se un tasto è stato "appena premuto"
    ...
    # Legge il valore grezzo di un asse analogico (da -1.0 a 1.0). Restituisce 0.0 in caso di errore
    def _axis(self, idx: int) -> float:
        if self.joy is None or idx >= self.joy.get_numaxes():
            return 0.0
        return self.joy.get_axis(idx)

    # Verifica se un pulsante è premuto in questo preciso istante (True/False)
    ...
    def read(self) -> dict:
        ...
        # Applico un filtro per ammorbidire le sterzate scattose del pollice
        ...
        # Restituisco tutti i calcoli fatti
        return {
            "steer":   steer,
            "accel":   accel,
            "brake":   brake,
